    ...
    const fetchTerms = async () => {
      try {
        ...
        const response = await apiFetch.get(
          `/connection/show_terms_reverse/?username=${connection.guest_user.username}&locker_name=${connection.guest_locker.name}&connection_name=${connection.connection_name}`
        );
        if (!response.status >= 200 && !response.status < 300) {
          throw new Error("Failed to fetch terms");
        }
...
const handleCloseModalClose = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (!updateResponse.status >= 200 && !updateResponse.status < 300) {
        const errorText = updateResponse.text();
  ...
  const updateXnode = async (resource) => {
    try {
      // const token = Cookies.get("authToken");
      const response = await apiFetch.post(`/resource/update-inode/`,
        {
  ...
  const handleAcceptResource = async () => {
    try {
      ...
      const token = Cookies.get("authToken");
      const response = await apiFetch.post(
        `/sharing/transfer-resource-host/`,
          {
            connection_name: conndetails.connection_name,
            host_locker_name: conndetails.host_locker.name,
            guest_locker_name: conndetails.guest_locker.name,
  ...
  const handleShareResource = async () => {
    try {
      ...
      // const token = Cookies.get("authToken");
      const response = await apiFetch.post(
        `/sharing/share-resource-approve-host/`,
        {
            connection_name: conndetails.connection_name,
            host_locker_name: conndetails.host_locker.name,
            guest_locker_name: conndetails.guest_locker.name,
  ...
  const handleConferResource = async () => {
    try {
      ...
      // const token = Cookies.get("authToken");
      const response = await apiFetch.post(
        `/sharing/confer-resource-approve-host/`,
        {
            connection_name: conndetails.connection_name,
            host_locker_name: conndetails.host_locker.name,
            guest_locker_name: conndetails.guest_locker.name,
  ...
  const handleCollateralResource = async () => {
    try {
      ...
      const response = await apiFetch.post(
        `/sharing/collateral-resource-host/`,
        {
          connection_name: conndetails.connection_name,
          host_locker_name: conndetails.host_locker.name,
          guest_locker_name: conndetails.guest_locker.name,
