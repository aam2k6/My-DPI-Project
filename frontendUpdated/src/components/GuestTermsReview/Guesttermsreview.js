...
export const Guesttermsreview = () => {
  ...
  const handleShareResource = async (resource) => {
    ...
    try {
      const token = Cookies.get("authToken");
      const response = await fetch(
        `host/share-reshare-vnode/`.replace(/host/, frontend_host),
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleResourceClick = (filePath) => {
    const url = `host/media/documents/${filePath}`.replace(
      /host/,
      frontend_host
    );
    window.open(url, "_blank");
  ...
  const content = (
    <>
      ...
      <div className="connection-details">
        ...
        <br></br>
        Host: {conndetails?.host_user?.username || "Loading..."} &lt;&gt; Guest:{" "}
        {conndetails?.guest_user?.username || "Loading..."}
      </div>
    </>
  );
  console.log("res", res);
