        ...
        const fetchTerms = async () => {
            try {
                ...
                const response = await fetch(
                    `http://172.16.192.201:8000/get-terms-value/?username=${hostUserUsername}&locker_name=${guestLockerName}&connection_name=${connectionName}`,
                    {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json",
        ...
        if (selectedLocker) {
            const fetchResources = async () => {
                try {
                    const token = Cookies.get("authToken");
                    const response = await fetch(
                        `http://172.16.192.201:8000/get-resources-user-locker/?locker_name=${selectedLocker}`,
                        {
                            method: "GET",
                            headers: {
                                Authorization: `Basic ${token}`,
    ...
    const handleSubmit = async () => {
        try {
            ...
            const updateResponse = await fetch(
                `http://172.16.192.201:8000/update-connection-terms/`,
                {
                    method: "PATCH",
                    headers: {
                        "Content-Type": "application/json",
